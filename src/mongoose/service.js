...
class Service extends AdapterService {
  constructor (options) {
    ...
    this.discriminatorKey = this.getModel(params).schema.options.discriminatorKey;
    this.discriminators = {};
  ...
  getModel (params) {
    ...
    if(params && params.mongooseClient) {
      model = params.mongooseClient.model(this.options.Model.modelName, this.options.Model.schema);
    }
  ...
  _find (params = {}) {
    ...
    const model = this.discriminators[discriminator] || this.getModel(params);
  ...
  _create (_data, params = {}) {
    ...
    return model.create(data, params.mongoose).then(results => {
      ...
      if ($populate && this.options.whitelist.includes('$populate')) {
        return Promise.all(results.map(result => this.getModel(params).populate(result, $populate)));
      }
      return results;
    }).then(results => {
      if (this.lean) {
  ...
  _remove (id, params = {}) {
    ...
    return this._getOrFind(id, findParams).then((data) => {
      if (id !== null) {
        return this.getModel(params).deleteOne(query, params.mongoose)
          .lean(this.lean)
          .exec()
          .then(() => data)
      ...
      return this.getModel(params).deleteMany(query, params.mongoose)
        .lean(this.lean)
        .exec()
        .then(() => data)
        .then(select(params, this.id));
